...
#   PVE_POOL        resource pool to create VMs in (default vmtest) -- the
#                   token's privileges must be granted here, not per-VMID; see
#                   the note below
...
# The resource pool the test VMs are created into, and where the token's
# privileges are granted.
#
# This is not cosmetic. Destroying a VM removes its VM-specific permissions, so
# a token granted PVEVMAdmin on /vms/9001 loses that grant the moment the suite
# cleans up after itself: the first run works and every later one fails with a
# bare 403. Found the hard way against a real node. Create checks VM.Allocate
# on /vms/<vmid> *or* on /pool/<pool>, and a pool ACL survives VM deletion, so
# the pool is what makes the suite repeatable.
#
# The trade-off, stated plainly: a pool grant authorises creating VMs in that
# pool rather than only the VMID band, so the band is enforced by this script
# rather than by Proxmox. What the ACL still guarantees is that the token
# cannot touch any pre-existing VM, because none of them are in the pool.
PVE_POOL=${PVE_POOL:-vmtest}
...
vm_define () {
    ...
    args+=(-d "vmid=$vmid" -d "name=$name" -d "tags=$PVE_TAG" -d "pool=$PVE_POOL")
    ...
    # model=virtio, spelled out. `net0=virtio,bridge=...` is valid in a config
    # file, where a bare first value means the model, but the API parses it as a
    # property string and rejects it: "duplicate key in comma-separated list
    # property: model".
    args+=(--data-urlencode "net0=model=virtio,bridge=$PVE_BRIDGE")
...
vm_serial_ready () { # vm_serial_ready NAME PORT TRIES
    ...
    while (( tries-- > 0 )); do
        ...
        if ssh -T -o BatchMode=yes -o ConnectTimeout=5 "$(_pve_ssh)" \
            ...
            # Hand expect a single executable path, not a command line.
            #
            # expect's `spawn` parses leading dash-arguments as its own options,
            # and `-o BatchMode=yes` was being swallowed as an abbreviation of
            # its `-open` flag: spawn appeared to succeed, but spawn_id ended up
            # holding the rest of the string, which surfaced later as
            # "spawn id o BatchMode=yes ... not open". Wrapping the command in a
            # script removes every dash from the spawn line.
            local dest helper
            dest=$(_pve_ssh)
            helper="${TMPDIR:-/tmp}/nixinst-serial-$vmid.sh"
            cat > "$helper" <<HELPER
#!/bin/sh
# Generated by tests/vm/lib-proxmox.sh. Attaches to the guest's serial console.
# -T so ssh allocates no remote pty: a pty would translate \n to \r\n and
# corrupt the transcript. A bare "socat -" for the same reason.
exec ssh -T -o BatchMode=yes $dest socat - UNIX-CONNECT:/var/run/qemu-server/$vmid.serial0
HELPER
            chmod +x "$helper"
            VM_SERIAL_CMD="$helper"
